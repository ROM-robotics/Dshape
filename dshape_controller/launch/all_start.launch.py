...
def generate_launch_description():
    ...
    bot = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(get_package_share_directory('dshape_controller'), 'launch', 'dshape_server.launch.py')])
    )

    twist_mux = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(get_package_share_directory('dshape_controller'), 'launch', 'twist_mux.launch.py')])
    )
    ...
    rviz_config_file = PathJoinSubstitution(
        [FindPackageShare("dshape_controller"), "rviz", "tunning.rviz"]
    )
    ...
    lidar = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(get_package_share_directory('dshape_controller'), 'launch', 'rplidar.launch.py')]),
        condition=IfCondition(use_lidar)
    )

    slamtoolbox_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(get_package_share_directory('dshape_nav2'), 'launch', 'online_async_launch_mapping_launch.py')]),
        condition=IfCondition(mapping)
    )

    amcl_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(get_package_share_directory('dshape_nav2'), 'launch', 'hw_localization_init_pose_launch.py')]),
        condition=IfCondition(amcl)
    )

    navigation_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(get_package_share_directory('dshape_nav2'), 'launch', 'hw_navigation_launch.py')]),
